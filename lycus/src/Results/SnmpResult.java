...
public class SnmpResult extends BaseResult {
	...
	public SnmpResult(String runnableProbeId, long timestamp, String data) {
		...
		// }
		// default:
		// Logit.LogError("SnmpResults - SnmpResults", "Unable to determine snmp
		// probe type: " + runnableProbeId + ", RP results didn't
		// initialized!");
		// }
	}
	...
	// checkIfTriggerd();
	// } catch (Exception e) {
	// Logit.LogError("SnmpResults - acceptResults", "Error triggering
	...
	// if (stringData == null) {
	// Logit.LogError("SnmpResults - acceptTextResults", "Error parsing snmp
	// probe results: " + getRunnableProbeId());
	...
	// private void acceptNumericResults(long lastTimestamp, ArrayList<Object>
	// results) {
	// Double data = null;
	// try {
	// data = Double.parseDouble((String) results.get(1));
	// } catch (Exception nfe) {
	// Logit.LogError("SnmpResults - acceptNumericResults", "Failed parsing snmp
	// probe results: " + getRunnableProbeId());
	...
	// private Double getBytesPerSecond(long lastTimestamp, Double data, long
	// ifSpeed) {
	...
	//
	// Double bandwidth = ((newOctets - oldOctets) * 8 * 100) / (((newTimestamp
	// - oldTimestamp) / 1000) * ifSpeed);
	//
	// return bandwidth;
	// }
	//
	...
	// this.addRollupsFromExistingMemoryDump(this.getDataRollups(),
	// existing[0]);
	// }
	...
	// if
	// (rpStr.contains("0b05919c-6cc0-42cc-a74b-de3b0dcd4a2a@788b1b9e-d753-4dfa-ac46-61c4374eeb84@inner_b0fb65d1-c50d-4639-a728-0f173588f56b"))
	// System.out.println("BREAKPOINT");
	//
	...
	//
	// if (((SnmpProbe) this.getRp().getProbe()).getDataType() ==
	// SnmpDataType.Numeric) {
	...
	// this.setStringData(null);
	// }
	// results.put("RAW@data@" + this.getLastTimestamp(),
	// rawResults.toJSONString());
	// break;
	// case Numeric:
	...
	// DataPointsRollup finishedDataRollup =
	// currentDataRollup.getLastFinishedRollup();
	//
	// if (currentDataRollup == null) {
	// SysLogger
	// .Record(new Log("Wrong Rollup Tables Number Of: " + getRunnableProbeId(),
	// LogType.Debug));
	// continue;
	// }
	...
	//
	// results.put("ROLLUP" + finishedDataRollup.getTimePeriod().getName() +
	// "@data@"
